...
sohomap2=''#path of the imaging soho map
sohomap1=''#path of the soho map before running difference time
stamap2=''#path of the imaging Stereo-A map
stamap1=''#path of the Stereo-A map before running difference time
...
def load_image(spacecraft: str, detector: str, date: dt.datetime, runndiff: bool):
    ...
    f = image(date,observatory,instrument,detector)
    #download_helioviewer(date, observatory, instrument, detector)
...
def image(date, observatory, instrument, detector):
    if observatory == 'SOHO':
        print('SOHO')
        lasco_map2=Map(sohomap2)
        lasco_map=Map(sohomap1)
        expt1=lasco_map.fits_header['EXPTIME']/50
        expt2=lasco_map2.fits_header['EXPTIME']/50
        f=Map(lasco_map2.data.astype(float)/expt2-lasco_map.data.astype(float)/expt1,lasco_map2.meta)
        ...
        print('STA')
        lasco_map2=Map(stamap2)
        lasco_map=Map(stamap1)
        expt1=lasco_map.fits_header['EXPTIME']/50
        expt2=lasco_map2.fits_header['EXPTIME']/50
        f=Map(lasco_map2.data.astype(float)/expt2-lasco_map.data.astype(float)/expt1,lasco_map2.meta)
    f=f.rotate(order=3)
...
class GCSGui(QtWidgets.QMainWindow):
    ...
    def create_widgets(self):
        ...
        self._s_height = SliderAndTextbox('Apex Height, R\u2090\u209A\u2091\u2093 [Rs]', 0, 100, params['height'])#24, params['height'])
    ...
    def make_plot(self):
        ...
        for i, sc in enumerate(spacecraft):
            ...
            image.plot(axes=ax, cmap='Greys_r', norm=colors.Normalize(vmin=-100, vmax=100))
            image.draw_limb(axes=ax,linewidth=0.5)
    ...
    def plot_mesh(self):
        ...
        for i, (image, ax) in enumerate(zip(self._images, self._axes)):
            if len(self._mesh_plots) <= i:
                ...
                params = {
                    'grid': dict(lw=0.5,alpha=0.5),
                    'point cloud': dict(ms=2)
                }[draw_mode]
                p = ax.plot_coord(mesh, style, color='cyan', scalex=False, scaley=False, **params)[0]
        ...
        #fig.savefig('/Volumes/金乌3号/storm_corona/panel2.pdf',dpi=100)
        fig.canvas.draw()
