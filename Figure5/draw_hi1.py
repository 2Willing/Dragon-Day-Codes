...
replace=1
for i in range(79-6,len(files)-6):
    lasco_map = Map(files[i])
    lasco_map2=Map(files[i+6])
    if replace==0 and 'HI1A_'+str(lasco_map2.date)+'.png' in files_all:
        continue
    ...
    expt1=lasco_map.fits_header['EXPTIME']/1200.
    expt2=lasco_map2.fits_header['EXPTIME']/1200.
    ...
    masked_lasco.plot(norm=colors.Normalize(vmin=-100, vmax=100), cmap=occult_colormap, axes=ax2)
    ...
    plt.savefig('HI1A_'+str(masked_lasco.date)+'.png',dpi=100)
    plt.close()
    ...
    masked_lasco.plot(norm=colors.Normalize(vmin=-200, vmax=200), cmap=occult_colormap, axes=ax2)
    masked_lasco.draw_limb()
    ...
    ax2.set_title(masked_lasco.name.replace('white-light ','Rdiff.'),fontsize=25,fontweight='bold',y=0.02,x=0.4,color='cyan')
    ax2.grid(False)
    ...
    plt.savefig('HI1A_'+str(masked_lasco.date)+'.pdf',dpi=100)
    plt.show()
    #plt.show()
    plt.close()
    break
