...
class SolarMACH():
    ...
    def plot(self, plot_spirals=True,
        ...
        """
        hide_logo = True  # optional later keyword to hide logo on figure
        AU = const.au / 1000  # km
        ...
        for i, body_id in enumerate(self.body_dict):
            ...
            if markers:
                ...
                if body_id[:6] == 'STEREO':
                    ...
                    front=27.
                    ang_a=np.deg2rad(end-body_long)
                    ang_front=np.deg2rad(front)
                    ang_dist=np.pi-ang_a-ang_front
                    dist_a=dist_body*np.cos(np.deg2rad(body_lat))
                    dist_end=dist_a/np.sin(ang_dist)*np.sin(ang_front)
                    ax.plot([np.deg2rad(body_long),np.deg2rad(end)],[dist_body*np.cos(np.deg2rad(body_lat)),dist_end],linestyle='-',color='red')
        ...
        if self.reference_long is not None:
            ...
            ref_arr = plt.arrow(np.deg2rad(delta_ref), 0.01, 0, arrow_dist, head_width=0.2, head_length=0.07, edgecolor='black',
                                facecolor='black', lw=1.8, zorder=5, overhang=0.2)
            thetas_fr = np.linspace(0, 2*np.pi, 100)
            r_frb=np.sqrt(r_fr**2+arrow_dist**2-2*r_fr*arrow_dist*np.cos(thetas_fr))
            theta_frb=np.arcsin(np.sin(thetas_fr)/r_frb*r_fr)+np.deg2rad(delta_ref)
            ax.plot(theta_frb,r_frb,color='black',linewidth=2)
