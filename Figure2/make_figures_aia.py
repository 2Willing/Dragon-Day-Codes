...
if rd==0:
    ...
    plt.title(map_i.name.replace('.0 Angstrom',r' ${\rm \AA}$'),y=0.92,color='white',fontsize=18,fontweight='bold')
    plt.gca().grid(False)
    ...
    plt.ylabel('Solar-Y',fontsize=15,fontweight='bold')
    plt.subplots_adjust(left=0.15,bottom=0.1,top=0.98,right=1,wspace=0,hspace=0)
    plt.title('Base Diff. '+map_i.name.replace('AIA '+wvl+'.0 Angstrom ',''),y=0.92,color='cyan',fontsize=18,fontweight='bold')#.replace('.0 Angstrom',r' ${\rm \AA}$')
    plt.gca().grid(False)
    plt.savefig('./Figure2/'+map_i.name+typef,dpi=dpi)
    plt.close()
    #break