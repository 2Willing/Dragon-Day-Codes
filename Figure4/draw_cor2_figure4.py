...
for i in [316-7]:#range(len(files)-6):
    lasco_map = Map(files[i])
    lasco_map2=Map(files[i+7])
    ...
    masked_lasco.plot(norm=colors.Normalize(vmin=-100, vmax=100),cmap='afmhot')
    ...
    plt.title(masked_lasco.name.replace('white-light 2023-','').replace('2023-',''),x=0.3,y=0.01,color='white',fontsize=18,fontweight='bold')#.replace('.0 Angstrom',r' ${\rm \AA}$')
